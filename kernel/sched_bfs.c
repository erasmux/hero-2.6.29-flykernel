...
{
	struct cpu_usage_stat *cpustat = &kstat_this_cpu.cpustat;
	cputime_t one_jiffy_scaled = cputime_to_scaled(jiffies_to_cputime(1));
	cputime64_t tmp = cputime_to_cputime64(jiffies_to_cputime(1));

	p->stime_pc += pc;
	if (p->stime_pc >= 100) {
		p->stime_pc -= 100;
		p->stime = cputime_add(p->stime, jiffies_to_cputime(1));
		p->stimescaled = cputime_add(p->stimescaled, one_jiffy_scaled);
		account_group_system_time(p, jiffies_to_cputime(1));
			 ...
			 unsigned long pc, unsigned long ns)
{
	...
	if (p->utime_pc >= 100) {
		p->utime_pc -= 100;
		p->utime = cputime_add(p->utime, jiffies_to_cputime(1));
		p->utimescaled = cputime_add(p->utimescaled, one_jiffy_scaled);
		account_group_user_time(p, jiffies_to_cputime(1));
		acct_update_integrals(p);
	}
	p->sched_time += ns;
